use std::{error::Error, fs, path::Path, f32::RADIX};
...
impl BinaryDiagnostic {
    ...
    fn get_trick(search: Vec<Vec<u32>>, compare : fn(a:i32, b:i32)->u32) -> u32 {
        ...
        for i in 0..len {
            ...
            if search.len() > 1 {
                ...
                search = search.into_iter().filter(|b| {
                    b[i] == compare(h[i],search_len-h[i])
                }).collect();

            }
        }

        let trick = search[0].iter().enumerate().map(|(i,a)| -> i32 {
    ...
    pub fn new(diagnostic: Vec<Vec<u32>>) -> BinaryDiagnostic {
        ...
        let gamma = hist.iter().enumerate().map(|(i,a)| -> i32 {
        ...
        let oxygen = Self::get_trick(diagnostic.clone(), |a,b| {
            if a >= b {
                1
            } else {
                0
        ...
        let co2 = Self::get_trick(diagnostic.clone(), |a,b| {
            if a >= b {
                0
            } else {
                1
        ...
        BinaryDiagnostic { gamma, epsilon, oxygen, co2 }
    }

    pub fn from_file(input_path: &Path) -> Result<BinaryDiagnostic, Box<dyn Error>> {
...
#[cfg(test)]
mod tests {
    use crate::d03_binary_diagnostic::BinaryDiagnostic;

    #[test]
    fn test_string_to_vec() {
        let s = String::from("00100\n11110\n10110\n10111\n10101\n01111\n00111\n11100\n10000\n11001\n00010\n01010");
        let e = vec![
            vec![0,0,1,0,0_u32],
            vec![1,1,1,1,0_u32],
            vec![1,0,1,1,0_u32],
